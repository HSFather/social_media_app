...
export default function Accounts() {
  ...
  return (
    <div className="px-2">
      <Image user={user} uploadImage={uploadImage} deleteImage={deleteImage} />
      <Form user={user} editBio={editBio} />
    </div>
  );
}
...
function Image({ user, uploadImage, deleteImage }) {
  return (
    <div className="mb-4">
      ...
      />
      {user.image ? (
        <button type="button" className="text-red-400" onClick={deleteImage}>
          Delete image
        </button>
      ) : (
